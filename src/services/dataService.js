...
export const dataService = {
    ...
    requestRegistration: async function (data) {
        ...
        const payload = {
            ...
            fullName: String(data.name || data.fullName || (data.firstName ? `${data.firstName} ${data.lastName || ''}`.trim() : username) || '').trim(),
            firstName: String(data.firstName || '').trim() || null,
            lastName: String(data.lastName || '').trim() || null,
            password: String(data.password || '').trim(),
            pin: String(data.pin || '').trim() || null,
            role: normalizedRole.toUpperCase(),
            ...
            address: String(data.address || data.shopAddress || data.permanentAddress || data.addressLine1 || '').trim() || null,
            shopAddress: String(data.shopAddress || data.address || '').trim() || null,
            permanentAddress: String(data.permanentAddress || '').trim() || null,
            businessName: String(data.businessName || data.shopName || '').trim() || null,
            businessType: String(data.businessType || 'Retail Store').trim() || null,
            ...
            fatherName: String(data.fatherName || '').trim() || null,
            gender: String(data.gender || 'Male').trim() || null,
            gstNumber: String(data.gstNumber || '').trim() || null,
            aadhaarNumber: String(data.aadhaarNumber || '').trim() || null,
            panNumber: String(data.panNumber || '').trim() || null,
            bankName: String(data.bankName || '').trim() || null,
            bankAccountHolder: String(data.bankAccountHolder || '').trim() || null,
            bankAccountNumber: String(data.bankAccountNumber || '').trim() || null,
            bankIfsc: String(data.bankIfsc || '').trim() || null,
            bankBranch: String(data.bankBranch || '').trim() || null,
            upiId: String(data.upiId || '').trim() || null,
            photoUrl: String(data.photoUrl || data.liveSelfieUrl || '').trim() || null,
            aadhaarPhotoUrl: String(data.aadhaarPhotoUrl || '').trim() || null,
            aadhaarBackPhotoUrl: String(data.aadhaarBackPhotoUrl || '').trim() || null,
            panPhotoUrl: String(data.panPhotoUrl || '').trim() || null,
            shopPhotoUrl: String(data.shopPhotoUrl || '').trim() || null,
            bankPassbookUrl: String(data.bankPassbookUrl || '').trim() || null,
            liveSelfieUrl: String(data.liveSelfieUrl || '').trim() || null,
            voterIdUrl: String(data.voterIdUrl || data.electricityBillUrl || '').trim() || null,
            drivingLicenceUrl: String(data.drivingLicenceUrl || '').trim() || null,
            passportUrl: String(data.passportUrl || '').trim() || null,
            gpsLat: String(data.gpsLat || '').trim() || null,
            gpsLong: String(data.gpsLong || '').trim() || null,
            deviceInfo: String(data.deviceInfo || '').trim() || null
        };
    ...
    adjustUserWalletBalance: function (userId, amount, type = 'CREDIT', remark = '') {
        try {
            ...
            // Check if user is in local storage list
            let userIndex = data.users.findIndex(u =>
                u.id == userId || u._id == userId || u.username === userId || u.mobile === userId || u.userId == userId
            );

            // If not found in mock data.users, look in the cached user list from WalletManager loadWallets
            if (userIndex === -1) {
                try {
                    const cacheRaw = localStorage.getItem('rupiksha_users_cache');
                    if (cacheRaw) {
                        const cachedUsers = JSON.parse(cacheRaw);
                        const matched = cachedUsers.find(u =>
                            u.id == userId || u._id == userId || u.username === userId || u.mobile === userId || u.userId == userId
                        );
                        if (matched) {
                            // Add user to local database list dynamically
                            data.users.push({
                                ...matched,
                                balance: String(matched.balance || 0),
                                walletBalance: String(matched.balance || 0)
                            });
                            userIndex = data.users.length - 1;
                        }
                    }
                } catch (e) {
                    console.warn('[adjustUserWalletBalance] cache read error:', e);
                }
            }

            // If still not found, check active session keys
            if (userIndex === -1) {
                for (const key of ['rupiksha_user', 'rupiksha_imp_user']) {
                    ...
                    if (raw) {
                        try {
                            ...
                            if (u.id == userId || u._id == userId || u.username === userId || u.mobile === userId || u.userId == userId) {
                                data.users.push({
                                    id: u.id || userId,
                                    ...
                                    walletBalance: String(u.balance || 0)
                                });
                                userIndex = data.users.length - 1;
                                break;
                            }
            ...
            // Ultimate fallback: add dynamic user template so operation succeeds
            if (userIndex === -1) {
                data.users.push({
                    id: userId,
                    username: userId,
            ...
            // Update active session if this user is currently logged in
            ...
            updateSessionKey('rupiksha_imp_user');

            // Log wallet transaction entry with correct opening/closing balance (no NaNs!)
            ...
            data.transactions.unshift({
                ...
                closingBalance: parseFloat(newBal),
            ...
            // Trigger same-tab storage events for local tab sync
            try {
                localStorage.setItem(`rupiksha_wallet_trigger_${Date.now()}`, newBal);
            } catch (_) {}
