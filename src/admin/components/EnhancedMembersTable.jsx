...
const BACKEND_URL = (import.meta.env.VITE_BACKEND_URL || '/api/v1').replace(/\/$/, '');
                    ...
                    <table className="w-full min-w-[1100px] border-collapse">
                        <thead>
                            <tr className="bg-gradient-to-r from-slate-50 to-slate-100 border-b-2 border-slate-200">
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[180px] border-r border-slate-200">Member</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[120px] border-r border-slate-200">Username</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[120px] border-r border-slate-200">Outlet</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[130px] border-r border-slate-200">Address</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[110px] border-r border-slate-200">Phone</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[150px] border-r border-slate-200">Email</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[100px] border-r border-slate-200">Owner</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[120px] border-r border-slate-200">Role</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[80px] border-r border-slate-200">Wallet</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[100px] border-r border-slate-200">Last AEPS</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[100px] border-r border-slate-200">Joined</th>
                                <th className="px-4 py-3.5 text-center text-[11px] font-extrabold text-slate-500 uppercase tracking-widest w-[130px]">Actions</th>
                            </tr>
                        </thead>
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 border-r border-slate-100">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    {member.partyCode
                                                        ? <span className="text-[10px] font-mono font-semibold text-indigo-500 bg-indigo-50 px-1.5 py-0.5 rounded">{member.partyCode}</span>
                                                        : <span className="text-[10px] text-slate-400">No party code</span>}
                                                </div>
                                            </div>
                                        </td>
                                        {/* Username */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <span className="text-xs font-mono font-semibold text-slate-600 bg-slate-100 px-2 py-1 rounded-md">{member.username}</span>
                                        </td>
                                        {/* Outlet */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <span className="text-sm text-slate-600 truncate block max-w-[110px] mx-auto" title={member.businessName}>{member.businessName || <span className="text-slate-300">—</span>}</span>
                                        </td>
                                        {/* Address */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <span className="text-xs text-slate-500 truncate block max-w-[120px] mx-auto" title={[member.addressLine1, member.city, member.stateName].filter(Boolean).join(', ')}>
                                                {[member.city, member.stateName].filter(Boolean).join(', ') || member.addressLine1 || <span className="text-slate-300">—</span>}
                                            </span>
                                        </td>
                                        {/* Phone */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <span className="text-sm font-medium text-slate-700 whitespace-nowrap">{member.mobile || '—'}</span>
                                        </td>
                                        {/* Email */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <span className="text-xs text-slate-500 truncate block max-w-[140px] mx-auto" title={member.email}>{member.email || '—'}</span>
                                        </td>
                                        {/* Owner */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <div className="flex flex-col gap-0.5 items-center">
                                                <span className="text-xs font-semibold text-slate-600 whitespace-nowrap">{ownerLabel}</span>
                                                {member.addedByPartyCode && <span className="text-[9px] font-mono text-indigo-400">{member.addedByPartyCode}</span>}
                                            </div>
                                        </td>
                                        {/* Role */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold border whitespace-nowrap ${roleBadge.bg}`}>
                                                {roleBadge.label}
                                            </span>
                                        </td>
                                        {/* Wallet */}
                                        ...
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            {member.lastAepsTxnDate
                                                ? <span className="text-xs text-emerald-600 font-semibold whitespace-nowrap">{formatDate(member.lastAepsTxnDate)}</span>
                                                : <span className="text-xs text-slate-300">Never</span>}
                                        </td>
                                        {/* Joined */}
                                        <td className="px-4 py-3 text-center border-r border-slate-100">
                                            <span className="text-xs text-slate-500 whitespace-nowrap">{formatDate(member.createdAt)}</span>
                                        </td>
                                        {/* Actions */}
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-center gap-1">
                                                {[{
                                                    icon: <Zap size={13} />, action: () => handleLoginAsMember(member),
                                                    title: 'Login as Member', cls: 'bg-emerald-50 hover:bg-emerald-500 text-emerald-600 hover:text-white border border-emerald-200'
                                                },{
                                                    icon: <Package size={13} />, action: () => handleViewServices(member),
                                                    title: 'Services', cls: 'bg-sky-50 hover:bg-sky-500 text-sky-600 hover:text-white border border-sky-200'
                                                },{
                                                    icon: <Eye size={13} />, action: () => handleViewDetail(member),
                                                    title: 'View Details', cls: 'bg-indigo-50 hover:bg-indigo-500 text-indigo-600 hover:text-white border border-indigo-200'
                                                },{
                                                    icon: <Edit3 size={13} />, action: () => handleEditMember(member),
                                                    title: 'Edit', cls: 'bg-amber-50 hover:bg-amber-500 text-amber-600 hover:text-white border border-amber-200'
                                                },{
                                                    icon: <Trash2 size={13} />, action: () => handleDelete(member),
                                                    title: 'Delete', cls: 'bg-rose-50 hover:bg-rose-500 text-rose-600 hover:text-white border border-rose-200'
                                                }].map((btn, i) => (
                                                    <button key={i} onClick={btn.action} title={btn.title}
                                                        className={`p-1.5 rounded-lg transition-all duration-150 ${btn.cls}`}>
                                                        {btn.icon}
                                                    </button>
                                                ))}
                                            </div>
