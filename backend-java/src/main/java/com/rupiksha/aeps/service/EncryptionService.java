...
public class EncryptionService {
    ...
    private void deriveKeyAndIV(String passphrase, byte[] salt, byte[] key, byte[] iv) throws Exception {
        ...
        while (offset < 48) {
            ...
            md5.update(salt);
            hash = md5.digest();
            System.arraycopy(hash, 0, result, offset, Math.min(hash.length, 48 - offset));
            offset += hash.length;
        }
